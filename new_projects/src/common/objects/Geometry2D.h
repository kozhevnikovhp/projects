...
public:
	...
	void Init();
	LOGICAL IsInitialized() const { return m_fLeft < m_fRight && m_fBottom < m_fTop; }
	...
	LOGICAL IsInside(SVector2D &position) { return IsInside(position.X(), position.Y()); }
	LOGICAL IsInside(double x, double y) { return x >= m_fLeft && x <= m_fRight && y >= m_fBottom && y <= m_fTop; }
...
public:
	...
	LOGICAL IsFilled() { return m_bFilled; }
	void SetFilled(LOGICAL bFilled = LOGICAL_TRUE) { m_bFilled = bFilled; }
	void Unformat(const CTextString &str) { Unformat(str.GetText()); }

// Public overridables
public:
	virtual char *GetGeomTypeStr() = 0;
	virtual void RecalcExtents() = 0;
	virtual double CalcDistance(double x, double y) = 0;
	virtual void SnapToGrid(double fGridSize) = 0;
	virtual void Format(CTextString &str) = 0;
...
public:
	...
	void SetLengthAngleDegrees(double fLength, double fAngle) { SetLengthAngleRadians(fLength, DEGREES_2_RADIANS(fAngle)); }
...
// Public overridables
public:
	virtual char *GetGeomTypeStr() { return "LINE_2D"; }
	virtual void RecalcExtents();
	virtual double CalcDistance(double x, double y);
	virtual void SnapToGrid(double fGridSize);
	virtual void Format(CTextString &str);
...
//	void SetAngleDegrees(double fAngle) { SetAngleRadians(DEGREES_2_RADIANS(fAngle)); }
	...
	LOGICAL IsPointInside(double x, double y);

// Public overridables
public:
	virtual char *GetGeomTypeStr() { return "RECTANGLE_2D"; }
	virtual void RecalcExtents();
	virtual double CalcDistance(double x, double y);
	virtual void SnapToGrid(double fGridSize);
	virtual void Format(CTextString &str);
