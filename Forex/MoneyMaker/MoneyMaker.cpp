// MoneyMaker.cpp : Defines the entry point for the console application.
//
...
class cMoneyMakerSession: public cNpbSession
{
public:
	cMoneyMakerSession();
...
cMoneyMakerSession::cMoneyMakerSession()
{
	m_iCurrentPair = 0;
}

void cMoneyMakerSession::Read()
{
	char szWorkingDir[MAX_FILE_NAME_LEN];
	char szTemplate[MAX_FILE_NAME_LEN];
	::GetCurrentDirectory(MAX_FILE_NAME_LEN-32, szWorkingDir);
	strcat(szWorkingDir, "\\");
	strcpy(szTemplate, szWorkingDir);
	strcat(szTemplate, "*.*");
	WIN32_FIND_DATA fd;
	...
	while (bFound)
	{
			...
			strcmp(fd.cFileName, ".."))
		{
			...
			{
				cCurrencyPair &pair = m_Pairs.Find(fd.cFileName);
				pair.Start();
			}
		}
...
void cMoneyMakerSession::ImproveSomebody()
{
	CStackCriticalSection section(&m_CriticalSection, "Improve");

	time_t CurrentTime; time(&CurrentTime);
	time_t deltaTime = CurrentTime - m_LastQuoteTime;
	if (deltaTime < 30)
	{
		printf("%d\r", deltaTime);
		return; // this seems working hour, do not occupy CPU with improvements
	}

	for (int i = 0; i < m_Pairs.size(); i++)
	{
		++m_iCurrentPair;
		int iPair = m_iCurrentPair % m_Pairs.size();
		cCurrencyPair &pair = m_Pairs[iPair];
		if (pair.Improve())
			break;
	}
}

void cMoneyMakerSession::Bye()
{
	CStackCriticalSection section(&m_CriticalSection, "Bye");
}

void
cMoneyMakerSession::onStatusMessage(const char *msg)
{
	CStackCriticalSection section(&m_CriticalSection, "StatusMessage");
	fprintf(stderr, "%s\n", msg);
	fflush(stderr);
}

void
cMoneyMakerSession::onTickersListChange(size_t n, const TickerInfo* pTickers)
{
	CStackCriticalSection section(&m_CriticalSection, "onTickersListChange");
	fprintf(stderr, "onTickersListChange: %d info(s) received\n", n);
}

void
cMoneyMakerSession::onTradeableTickersListChange(size_t n,
											  const TradeableTickerInfo* ti)
{
	CStackCriticalSection section(&m_CriticalSection, "onTradeableTickersListChange");
...
void
cMoneyMakerSession::onQuotesChange(int n, const QuoteChangeEvent *qce)
{
	CStackCriticalSection section(&m_CriticalSection, "onQuoteChange");
	...
	for (int i = 0; i < n; i++)
	{
		...
		SDecision *pDecision = pair.WhatToDoNow(qce[i]);
		if (pDecision->decision == DO_NOTHING)
		{
			//printf("DO NOTHING %s\n", pair.getTicker());
		}
		else if (pDecision->decision == DO_BUY)
		{
			printf("BUY %s %s for %s, limit = %s\n", pDecision->szAmount, pair.getTicker(), qce[i].offer, pDecision->szLimit);
			ReqID = doStreamOperation(AMFX_OP_BUY, qce[i].ticker, pDecision->szAmount,
 											  qce[i].offer, pDecision->szLimit, "Buy", &err);
			if (err == RQP_ACCEPTED)
			{
				pair.CommitBuy(pDecision);
			}
			else
			{
				printf("Error during buy %d\n", err);
			}
		}
		else if (pDecision->decision == DO_SELL)
		{
			printf("SELL %s %s for %s, limit = %s\n", pDecision->szAmount, pair.getTicker(), qce[i].bid, pDecision->szLimit);
			ReqID = doStreamOperation(AMFX_OP_SELL, qce[i].ticker, pDecision->szAmount,
 											  qce[i].bid, pDecision->szLimit, "Sell", &err);
			if (err == RQP_ACCEPTED)
			{
				pair.CommitSell(pDecision);
			}
			else
			{
				printf("Error during sell %d\n", err);
			}
		}
	}
}
...
int
cMoneyMakerSession::onConnectionEvent(AMFXConnectionStage stage, int flag)
{
	char buf[80];
	sprintf(buf, "onConnectionEvent: stage %d flag %d", stage, flag);
	onStatusMessage(buf);
	CStackCriticalSection section(&m_CriticalSection, "onConnectionEvent");
...
void
cMoneyMakerSession::onRequestProcessing(int requestId, AMFXRProcessingStatus st)
{
	printf("RequestId %d status %d\n", requestId, st);
}

void
cMoneyMakerSession::onTrade(int isOwned, const struct TradeInfo *pInfo)
{
	CStackCriticalSection section(&m_CriticalSection, "onTTrade");
	printf("Got %s trade event\n", (isOwned)?"OWNED":"NOT OWNED");
	fprintfTrade(stdout, pInfo);
	tradeAck(pInfo->ticket);

	char szTicker[32];
	sprintf(szTicker, "%s%s", pInfo->ccy1, pInfo->ccy2);
	cCurrencyPair &pair = m_Pairs.Find(szTicker);
	pair.OnTradeDone(isOwned, pInfo);

	FILE *pFile = fopen("AllTrades.txt", "a");
	if (pFile)
	{
...
void
cMoneyMakerSession::onChatMessage(int p, const char *sender,
							   const char *msg, size_t msgSize,
							   double ts)
{
	return;
...
void
cMoneyMakerSession::onChatMessageDelivery(int msgId, int isDelivered,
									   double timestamp)
{
...
void
cMoneyMakerSession::onMarketDealQuote(int requestId,
								   const char *bid, const char *offer,
								   int timeout, double timestamp)
{
...
void
cMoneyMakerSession::onTradeHistoryChange(int nChanges, const struct TradeInfo* ti)
{
	CStackCriticalSection section(&m_CriticalSection, "onTradeHistoryChange");
...
void
cMoneyMakerSession::onTradeHistoryRemove(int nChanges, const char *removed[])
{
	CStackCriticalSection section(&m_CriticalSection, "onTickersListRemove");
...
void
cMoneyMakerSession::onOrdersListChange(int nChanges, const struct OrderInfo* oi)
{
	CStackCriticalSection section(&m_CriticalSection, "onOrderListChange");
...
void
cMoneyMakerSession::onOrdersListRemove(int nChanges, const int *removed)
{
	CStackCriticalSection section(&m_CriticalSection, "onOrderListRemove");
...
void
cMoneyMakerSession::onAccountingBaseInfo(int nInfos,
									  const AMFXAccountBaseInfo *infos)
{
	CStackCriticalSection section(&m_CriticalSection, "onAccountingBaseInfo");
...
void
cMoneyMakerSession::onOpenPositionsChange(const char *accountId,
									  int n, const OpenPosition* ps)
{
	CStackCriticalSection section(&m_CriticalSection, "onOpenPositionChange");
	for (int i=0; i<n; i++){
...
void
cMoneyMakerSession::onNettingChange(const char *accountId,
								 int n, const CcyAmount*ccys)
{
	CStackCriticalSection section(&m_CriticalSection, "onNettingChange");
	for (int i=0; i<n; i++){
...
void
cMoneyMakerSession::onBalanceChange(const char *accountId,
								 int n, const CcyAmount*ccys)
{
	CStackCriticalSection section(&m_CriticalSection, "onBalanceChange");
	for (int i=0; i<n; i++){
...
void showOrdersOps(cMoneyMakerSession *session)
{
	AMFXRProcessingStatus err;
	int rqid;
...
void cancelOrders(cMoneyMakerSession *session)
{
	getch();
	for(;;) {
		int oid;
...
void showChatOps(cMoneyMakerSession *session)
{
...
int main(int argc, char **argv)
{
	...
	cMoneyMakerSession *pSession = new cMoneyMakerSession();
	pSession->Read();
...
//	_getch();
	...
	pSession->Bye(); // to prevent destruction with uncompleted code controlled by critical section
	delete pSession;
