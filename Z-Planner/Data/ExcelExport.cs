...
namespace ZZero.ZPlanner.Translators
{
    ...
    class ExcelExporter
    {
        ...
        public void ExportClosedXML()
        {
            ...
            worksheet = workbook.Worksheets.Add(z.Title);
            ...
            var rngTable = worksheet.Range(2, 2, iLRow, iLCol + 2);
        ...
        private void ExportFooter(int iRow, int iCol)
        {
            ...
            for (int i = iRow; i < iRow + 5; i++)
            {
                ...
                worksheet.Range(i, iCol, i, iCol + 1).Merge();
                worksheet.Cell(i, iCol).Value = s;

                worksheet.Range(i, iCol + 2, i, iCol + 4).Merge();
            }
            worksheet.Cell(iRow, iCol + 2).Value = Options.TheOptions.Company;
            worksheet.Cell(iRow + 1, iCol + 2).Value = options.ProjectName;//project
            worksheet.Cell(iRow + 2, iCol + 2).Value = options.DesignRevision;
            worksheet.Cell(iRow + 3, iCol + 2).Value = Options.TheOptions.UserEMail;
            worksheet.Cell(iRow + 4, iCol + 2).Value = Path.GetFileName(ZPlannerManager.ProjectFile);//z0p file
            ...
            for (int i = iRow; i < iRow + 5; i++)
            {
                ...
                worksheet.Range(i, col, i, col + 1).Merge();
                worksheet.Cell(i, col).Value = s;

                worksheet.Range(i, col + 2, i, col + 5).Merge();
            }
        ...
        private void ExportStackup(out int rightColumn)
        {
            ...
            {
                AddHeader(headerRow, headerColumn, "Copper Weight" + Environment.NewLine + "(oz)");
                headerColumn++;
            }
            if (options.Contains(ExportFlags.Copper))
            {
                AddHeader(headerRow, headerColumn, "% Copper");
                headerColumn++;
            }
            ...
            {
                AddHeader(headerRow, headerColumn, "Rz-Bottom" + Environment.NewLine + "(um)");
                headerColumn++;
            }

            int impCol1 = headerColumn;
            ...
            {
                AddHeader(headerRow, headerColumn, "Plane" + Environment.NewLine + "Reference");
                headerColumn++;
            }
            ...
            for (int i = col1; i < layerColumn; i++)
            {
        ...
        {
            worksheet.Cell(headerRow, headerColumn).Style.Fill.BackgroundColor = XLColor.Black;//XLColor.AshGrey;
