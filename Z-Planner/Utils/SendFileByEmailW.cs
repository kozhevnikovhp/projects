...
namespace ZZero.ZPlanner.Utils
{
    ...
    class MAPIW
    {
        ...
        static extern int MAPISendMailW(IntPtr sess, IntPtr hwnd, MapiMessageW message, int flg, int rsv);

        int SendMail(string strSubject, string strBody, int how)
        {
            MapiMessageW msg = new MapiMessageW();
            ...
            m_lastError = MAPISendMailW(new IntPtr(0), new IntPtr(0), msg, how, 0);
            if (m_lastError > 1)
                MessageBox.Show("MAPISendMailW failed! " + GetLastError(), "MAPISendMailW");
        ...
        {
            MapiRecipDescW recipient = new MapiRecipDescW();
        ...
        IntPtr GetRecipients(out int recipCount)
        {
            ...
            int size = Marshal.SizeOf(typeof(MapiRecipDescW));
            IntPtr intPtr = Marshal.AllocHGlobal(m_recipients.Count * size);

            int ptr = (int)intPtr;
            foreach (MapiRecipDescW mapiDesc in m_recipients)
            {
        ...
        IntPtr GetAttachments(out int fileCount)
        {
            ...
            int size = Marshal.SizeOf(typeof(MapiFileDescW));
            IntPtr intPtr = Marshal.AllocHGlobal(m_attachments.Count * size);

            MapiFileDescW mapiFileDesc = new MapiFileDescW();
        ...
        void Cleanup(ref MapiMessageW msg)
        {
            int size = Marshal.SizeOf(typeof(MapiRecipDescW));
            int ptr = 0;
            ...
            if (msg.recips != IntPtr.Zero)
            {
                ...
                {
                    Marshal.DestroyStructure((IntPtr)ptr, typeof(MapiRecipDescW));
                    ptr += size;
                }
            ...
            {
                size = Marshal.SizeOf(typeof(MapiFileDescW));

                ptr = (int)msg.files;
                for (int i = 0; i < msg.fileCount; i++)
                {
                    Marshal.DestroyStructure((IntPtr)ptr, typeof(MapiFileDescW));
                    ptr += size;
                }
        ...
        List<MapiRecipDescW> m_recipients = new List<MapiRecipDescW>();
