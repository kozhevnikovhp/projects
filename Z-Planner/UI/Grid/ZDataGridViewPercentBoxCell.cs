...
{
    class ZDataGridViewPercentBoxCell : DataGridViewTextBoxCell, IZDataGridViewCell
    {
        #region Fields
        private int valueColumnSpan = 1;
        private int valueRowSpan = 1;
        private ZDataGridViewPercentBoxCell valueOwnerCell;
        #endregion

        public ZDataGridViewPercentBoxCell()
        {
        ...
        public DataGridViewCell OwnerCell
        {
            ...
            set { valueOwnerCell = value as ZDataGridViewPercentBoxCell; }
        }
        ...
        protected override object GetFormattedValue(object value, int rowIndex, ref DataGridViewCellStyle cellStyle, TypeConverter valueTypeConverter, TypeConverter formattedValueTypeConverter, DataGridViewDataErrorContexts context)
        {
            ...
            if (parameter != null) 
            {
                if (double.TryParse(value.ToString().Replace("%", "").Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out dValue))
                    return dValue.ToString(ZPlannerManager.GetFormatByParameter(parameter), CultureInfo.InvariantCulture);
            }
            else if (double.TryParse(value.ToString().Replace("%", "").Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out dValue))
                    return dValue.ToString("N", CultureInfo.InvariantCulture);
        ...
        protected override void Paint(Graphics graphics, Rectangle clipBounds, Rectangle cellBounds, int rowIndex, DataGridViewElementStates cellState, object value, object formattedValue, string errorText, DataGridViewCellStyle cellStyle, DataGridViewAdvancedBorderStyle advancedBorderStyle, DataGridViewPaintParts paintParts)
        {
            ...
            object percentValue = (value == null || value.ToString() == string.Empty || value.ToString().Contains("%")) ? value : value + "%";
            object percentFormatedValue = (formattedValue == null || formattedValue.ToString() == string.Empty || formattedValue.ToString().Contains("%")) ? formattedValue : formattedValue + "%";
            ...
            {
                NativePaint(graphics, clipBounds, cellBounds, rowIndex, cellState, percentValue, percentFormatedValue, errorText, cellStyle, advancedBorderStyle,
                        paintParts);
                return;
            }
            ...
            if (valueOwnerCell != null)
            {
                ...
                percentValue = (value == null || value.ToString() == string.Empty || value.ToString().Contains("%")) ? value : value + "%";
                percentFormatedValue = (formattedValue == null || formattedValue.ToString() == string.Empty || formattedValue.ToString().Contains("%")) ? formattedValue : formattedValue + "%";
            }
            ...
            using (var g = DataGridView.CreateGraphics())
            {
                ...
                ownerCell.NativePaint(g, clipBounds, cellBounds2, rowIndex, cellState,
                    percentValue, percentFormatedValue, errorText,
                ...
                if ((paintParts & DataGridViewPaintParts.Border) != DataGridViewPaintParts.None)
                {
                    ...
                    var rightBottomCell = DataGridView[columnIndex + columnSpan - 1, rowIndex + rowSpan - 1] as ZDataGridViewPercentBoxCell ?? this;
        ...
        private void SetSpan(int columnSpan, int rowSpan)
        {
            ...
            if (DataGridView != null)
            {
                ...
                for (int rowIndex = RowIndex; rowIndex < RowIndex + prevRowSpan; rowIndex++)
                    ...
                    {
                        var cell = DataGridView[columnIndex, rowIndex] as ZDataGridViewPercentBoxCell;
                        if (cell != null)
                            cell.OwnerCell = null;
                    }
        ...
        {
            //this.ToolTipText = (ValueType == null || value.ToString() == string.Empty) ? string.Empty : value.ToString().Replace("%", string.Empty).Trim() + "%";
            if (OwnerCell != null)
        ...
        protected override Rectangle BorderWidths(DataGridViewAdvancedBorderStyle advancedBorderStyle)
        {
            ...
            var rightBottomCell = DataGridView[
                ...
                RowIndex + RowSpan - 1] as ZDataGridViewPercentBoxCell;
        ...
        {
            double doubleVal;
            if (double.TryParse(formattedValue.ToString().Replace("%", "").Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out doubleVal)) return doubleVal;
            else return base.ParseFormattedValue(formattedValue.ToString().Replace("%", "").Trim(), cellStyle, formattedValueTypeConverter, valueTypeConverter);
        }
