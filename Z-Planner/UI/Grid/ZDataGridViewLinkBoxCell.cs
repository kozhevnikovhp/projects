...
{
    class ZDataGridViewLinkBoxCell : DataGridViewButtonCell, IZDataGridViewCell
    {
        #region Fields
        private int valueColumnSpan = 1;
        private int valueRowSpan = 1;
        private ZDataGridViewLinkBoxCell valueOwnerCell;
        #endregion

        public ZDataGridViewLinkBoxCell()
            : base()
        {
        ...
        public DataGridViewCell OwnerCell
        {
            ...
            set { valueOwnerCell = value as ZDataGridViewLinkBoxCell; }
        }
        ...
        protected override void Paint(Graphics graphics, Rectangle clipBounds, Rectangle cellBounds, int rowIndex, DataGridViewElementStates cellState, object value, object formattedValue, string errorText, DataGridViewCellStyle cellStyle, DataGridViewAdvancedBorderStyle advancedBorderStyle, DataGridViewPaintParts paintParts)
        {
            ...
            ButtonEnabled = false;
            if (layerParameter != null && layerParameter.Layer != null)
            {
                ZLayerType? layerType = layerParameter.Layer.GetLayerType();
                switch(this.OwningColumn.Name)
                {
                    ...
                    case ZStringConstants.ParameterIDZdiff_LossViewer:
                        value = "Loss Viewer";
                        isUsed = layerParameter.Layer.GetLayerParameterValue(ZStringConstants.ParameterIDZdiff_IsUsed);
                        if (isUsed == null || isUsed.ToLower() == "true")
                        {
                            if (layerType == ZLayerType.Signal || layerType == ZLayerType.SplitMixed)
                                ButtonEnabled = true;
            ...
            if (ButtonEnabled)
                this.Style = new DataGridViewCellStyle { Padding = new Padding(0, 0, 0, 0) };
            else
                this.Style = new DataGridViewCellStyle { Padding = new Padding(this.OwningColumn.Width, 0, 0, 0) };

            formattedValue = this.GetFormattedValue(value, rowIndex, ref cellStyle, null, null, DataGridViewDataErrorContexts.Display);
                ...
                || (OwnerCell == null && ColumnSpan == 1 && RowSpan == 1))
            {
                ...
                //graphics.DrawString(value, Control.DefaultFont, new SolidBrush(cellStyle.ForeColor), stringLocation);
                return;
            }
            ...
            using (var g = DataGridView.CreateGraphics())
            {
                ...
                if ((paintParts & DataGridViewPaintParts.Border) != DataGridViewPaintParts.None)
                {
                    ...
                    var rightBottomCell = DataGridView[columnIndex + columnSpan - 1, rowIndex + rowSpan - 1] as ZDataGridViewLinkBoxCell
                                          ?? this;
        ...
        private void SetSpan(int columnSpan, int rowSpan)
        {
            ...
            if (DataGridView != null)
            {
                ...
                for (int rowIndex = RowIndex; rowIndex < RowIndex + prevRowSpan; rowIndex++)
                    ...
                    {
                        var cell = DataGridView[columnIndex, rowIndex] as ZDataGridViewLinkBoxCell;
                        if (cell != null)
                            cell.OwnerCell = null;
                    }
        ...
        protected override Rectangle BorderWidths(DataGridViewAdvancedBorderStyle advancedBorderStyle)
        {
            ...
            var rightBottomCell = DataGridView[
                ...
                RowIndex + RowSpan - 1] as ZDataGridViewLinkBoxCell;
        ...
        public void ZLayerParameter_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            switch (e.PropertyName)
            {
                case "Value":
                    bool result;
                    if (bool.TryParse(layerParameter.Value, out result)) this.Value = result;
                    this.Value = false;
                    break;
                default:
                    break;
